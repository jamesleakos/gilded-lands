...
var RuntimeAbility = /** @class */ (function () {
    ...
    RuntimeAbility.fromRuntimeJSON = function (json) {
        return new RuntimeAbility(json.indexForUpgrades, json.name, RuntimeEffect_1.default.fromRuntimeJSON(json.effect), json.costs.map(function (cost) { return PayResourceCost_1.default.fromJSON(cost); }), json.usesPerTurn, json.usesRemaining, json.useableInPhases.map(function (phase) { return Phase_1.PhaseEnum[phase]; }), json.isActive, json.image);
    };
    RuntimeAbility.fromLibraryJSON = function (json) {
        return new RuntimeAbility(json.indexForUpgrades, json.name, RuntimeEffect_1.default.fromLibraryJSON(json.effect), json.costs.map(function (cost) { return PayResourceCost_1.default.fromJSON(cost); }), json.usesPerTurn, json.usesRemaining, json.useableInPhases.map(function (phase) { return Phase_1.PhaseEnum[phase]; }), json.isActive, json.image);
    };
    RuntimeAbility.isLibraryJSONValid = function (json) {
        ...
        if (!RuntimeEffect_1.default.isLibraryJSONValid(json.effect)) {
            console.log('Invalid JSON: effect is not valid');
            return false;
        }
        if (!Array.isArray(json.costs) ||
            !json.costs.every(function (c) { return PayResourceCost_1.default.isLibraryJSONValid(c); })) {
            console.log('Invalid JSON: costs is not an array');
            return false;
        }
        for (var _i = 0, _a = json.costs; _i < _a.length; _i++) {
            var cost = _a[_i];
            if (!PayResourceCost_1.default.isLibraryJSONValid(cost)) {
