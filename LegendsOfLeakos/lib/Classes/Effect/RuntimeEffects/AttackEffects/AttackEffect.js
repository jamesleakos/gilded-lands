...
var AttackEffect = /** @class */ (function (_super) {
    ...
    // target criteria
    AttackEffect.prototype.targetCriterias = function () {
        return [];
    };
    ...
    AttackEffect.prototype.areTargetsAvailable = function (state, sourceEntity) {
        var sourceCard = sourceEntity;
        if (!sourceCard)
            return false;
        var sourceCardZone = state.getZoneByInstanceId(sourceCard.residingZoneInstanceId);
        if (!Zone_1.ZoneEnumMethods.isBoard(sourceCardZone.zoneEnum))
            return false;
        // we can implement the rest of this if we want, but for now the server takes care of it
        return true;
    };
    AttackEffect.prototype.isAllTargetInfoValid = function (sourceEntity, state, targetInfoList) {
        return true;
    };
    // #endregion
