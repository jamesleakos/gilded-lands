...
const stringsToUrl = function (
  ...
  try {
    ...
    if (typeObj[depth].length > 0) {
      ...
      const randomIndex = Math.floor(Math.random() * typeObj[depth].length);
      url = typeObj[depth][randomIndex];
    } else {
      const randomIndex = Math.floor(Math.random() * typeObj.all.length);
    ...
    console.log('e: ', e);
    throw new Error(
      'type and depth must be strings, and type must be a valid land type - instead, type is ' +
        type +
        ' and depth is ' +
        depth
    );
...
const intsToUrl = function (
  ...
  if (
    ...
    throw new Error(
      'type and depth must be numbers, and type must be between 0-7 - instead, type is ' +
        typeof type +
        ' and depth is ' +
        typeof depth
    );
  ...
  const depthString: string = Enums.BiomeDepth[Math.min(depth, 3)];
